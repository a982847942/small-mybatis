package nuaa.zk.s09.datasource.pooled;

import nuaa.zk.s09.datasource.unpooled.UnpooledDataSource;
            ...
            synchronized (state) {
                ...
                // 如果无空闲链接：创建新的链接
                    ...
                    // 活跃连接数已满
                        ...
                        /**
                         * 为什么可以直接终止活跃的数据库连接？ 可能是因为用户使用完毕后没有调用close方法
                         * 导致连接一直处于active状态，其实没有什么用，资源浪费，因此可以使用poolMaximumCheckoutTime(用户自己设置)
                         * 与longestCheckoutTime来对比
                         */
